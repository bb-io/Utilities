...
namespace Apps.Utilities.Actions
{
    ...
    {
        [Action("Add provenance", Description = "Adds translation or review provenance to any file supported by Blackbird Filters and returns an XLIFF 2.2 file.")]
        public async Task<FileDto> AddProvenance(
            [ActionParameter] AddProvenanceRequest request)
        {
            if (request.File is null)
                throw new PluginMisconfigurationException("File is required. Please provide a supported file.");

            var provenanceType = request.ProvenanceType?.Trim().ToLowerInvariant();
            if (provenanceType is not (
                    ProvenanceTypeDataSourceHandler.Translation
                    or ProvenanceTypeDataSourceHandler.Review))
            {
                throw new PluginMisconfigurationException("Provenance type is required. Please select translation or review.");
            }
            ...
            {
                throw new PluginMisconfigurationException($"The file could not be parsed. Please provide a file type supported by Blackbird Filters. Details: {loadResult.Error}");
            }
            ...
            var transformations = new Stack<Transformation>([transformation]);

            while (transformations.TryPop(out var currentTransformation))
            {
                var provenance = provenanceType == ProvenanceTypeDataSourceHandler.Translation
                    ? currentTransformation.Provenance.Translation
                    : currentTransformation.Provenance.Review;

                provenance.Person = request.Person ?? provenance.Person;
                provenance.PersonReference = request.PersonReference ?? provenance.PersonReference;
                provenance.Organization = request.Organization ?? provenance.Organization;
                provenance.OrganizationReference = request.OrganizationReference ?? provenance.OrganizationReference;
                provenance.Tool = request.Tool ?? provenance.Tool;
                provenance.ToolReference = request.ToolReference ?? provenance.ToolReference;

                foreach (var child in currentTransformation.Children.OfType<Transformation>())
                    transformations.Push(child);
            }

            var serializedXliff = Xliff2Serializer.Serialize(
                transformation,
                Xliff2Version.Xliff22);
            ...
            var outputName = inputExtension.Equals(".xlf", StringComparison.OrdinalIgnoreCase)
            ...
            return new FileDto { File = outputFile };
        }
